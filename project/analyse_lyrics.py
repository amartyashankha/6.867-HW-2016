...
def test_classifiers(test_ratio = 0.05, norm = False, one_hot = True, feat_sel = 5000):
    ...
    for classifier in np.hstack((regressors,classifiers)):
        ...
        if classifier in classifiers:
            ...
            print "classification score: ",classifier.score(testX, testY, sample_weight = testW)
        else:
        ...
        print "MSE of prediction: ", metrics.mean_squared_error(testY, pred, sample_weight = testW)
        mode = np.bincount(testY).argmax()
        print "MSE of predicting mode: ", metrics.mean_squared_error(testY, [mode]*len(testY), sample_weight = testW)
